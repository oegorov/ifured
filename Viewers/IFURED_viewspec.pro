pro IFURED_viewspec_REGIONparam
...
x_min=min([x1,x2])/scX+zero_pix & x_max=max([x1,x2])/scX+zero_pix
y_min=min([y1,y2])/scY & y_max=max([y1,y2])/scY
if x_min lt 0 then x_min=0
if x_max gt zero_pix+Nx-1 then x_max=zero_pix+Nx-1
...
pro IFURED_viewspec_BOX
COMMON IFURED_viewspec_IMAblock
COMMON IFURED_viewspec_WINblock
...
pro IFURED_viewspec_DRAWIMAGE
COMMON IFURED_viewspec_IMAblock
COMMON IFURED_viewspec_RANGEblock
COMMON IFURED_viewspec_WINblock
COMMON IFURED_viewspec_BOXblock
loadct,0,/sil
WSET, win_num
...
pro IFURED_viewspec_MICRODRAW
COMMON IFURED_viewspec_WINblock
COMMON IFURED_viewspec_IMAblock
COMMON IFURED_viewspec_BOXblock
COMMON IFURED_viewspec_RANGEblock
COMMON IFURED_viewspec_Wbutton
...
micromap=congrid(reform(frame(x_2/scX-w:x_2/scX+w,y_2/scY-w:y_2/scY+w,plane)),150,150)
...
file_curr=file
file=dialog_pickfile(title='Load file',filter=['*.f*s'],get_path=wdir);file=pickfile(/read,filter='*.fts')
...
bin=sxpar(h,'binning')
if not keyword_set(bin) then bin=1 else begin
  bin=fix(str_sep(bin,'x')) & bin=float(bin(0))/bin(1)
endelse
sc=float(y_size)/float(Ny)
Naxis=sxpar(h,'NAXIS')
Nz=sxpar(h,'NAXIS3')
mm=minmax(where(finite(reform(frame[*,ny/2,plane]))))
nx=mm[1]+1-mm[0]
zero_pix=mm[0]
ima=congrid(frame(zero_pix:zero_pix+nx-1,*,plane),x_size,y_size)
...
PRO IFURED_viewspec_event, event

COMMON IFURED_viewspec_Wbase
COMMON IFURED_viewspec_LABELblock
COMMON IFURED_viewspec_SLIDEdlock
COMMON IFURED_viewspec_Wbutton
COMMON IFURED_viewspec_REG
COMMON IFURED_viewspec_IMAblock
COMMON IFURED_viewspec_RANGEblock
...
'DRAW_WIN_EVENT': BEGIN
      ...
      if event.release EQ 1 then begin
             ...
             x2=event.x  & y2=event.y
  			 IFURED_viewspec_REGIONparam
  			 if x1 eq x2 and y1 eq y2 then begin
  			  WIDGET_CONTROL,button3,SENSITIVE=0
  			  WIDGET_CONTROL,button4,SENSITIVE=0
  			  WIDGET_CONTROL,button5,SENSITIVE=0
  			  IFURED_viewspec_DRAWIMAGE
  			  if XREGISTERED('IFURED_viewspec_Wplot' ) gt 0 then  WIDGET_CONTROL,base_plot,/destroy
  			 endif else begin
      ...
      IF (event.press EQ 0) AND (event.release EQ 0) THEN BEGIN
            ...
            IFURED_viewspec_MICRODRAW
            if sw eq 1 then IFURED_viewspec_BOX

         	WIDGET_CONTROL,pixel_label, $
				   SET_VALUE='Pixel' + STRCOMPRESS('('+STRING(FIX(event.x/scX)+zero_pix)+','+$
				   STRING(FIX(event.y/scY))+')')

  			 if d_lambda gt 0 then $
  				WIDGET_CONTROL,wave_label, $
  				   SET_VALUE='wavelength' + STRCOMPRESS(STRING(FIX((event.x/scX+zero_pix)*d_lambda+lambda_0))+'A') $
  				   else WIDGET_CONTROL,wave_label,SET_VALUE='None'
...
PRO IFURED_viewspec, GROUP=GROUP
...
base_view = WIDGET_BASE(TITLE = 'VIEW '+file,/ROW,xoffset=520,yoffset=400);xsize=x_size+125,ysize=y_size+30,

BASE1 = WIDGET_BASE(base_view,col=1)
...
base3=WIDGET_BASE(base_view,col=1)
button1 = WIDGET_BUTTON(base3, $
    UVALUE = 'FILE', $
    VALUE = 'LOAD FILE',ys=22)

base4=WIDGET_BASE(base3,row=1)
slit_label = WIDGET_LABEL(base4, $
  VALUE='Slit:',font=titfont,ys=21)
pln_group = CW_BGROUP(base4,['Left','Right'],/EXCLUSIVE,uvalue='pln',set_value=plane,/row,/return_index,/no_release)
viewrange = WIDGET_SLIDER(base3, $
                        MINIMUM = 1, MAXIMUM = 100, VALUE = 50, $
      UVALUE = 'range')
contrast_label = WIDGET_LABEL(base3, $
  VALUE='Contrast',font=titfont,ys=21)
  
  
viewbackground = WIDGET_SLIDER(base3,  $
                        MINIMUM = 1, MAXIMUM = 100, VALUE = 50, $
      UVALUE = 'background')
background_label=widget_label(base3,val="Background",font=titfont,ys=21)

;background_label = WIDGET_LABEL(base3,  $
;  VALUE='background')
;label1=WIDGET_LABEL(base3,  $
; VALUE='  ')
pixel_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='None')

value_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='None')
wave_label=WIDGET_LABEL(base3, xs=150, $
    VALUE='None')
button3 = WIDGET_BUTTON(base3, $
    UVALUE = 'plot_region', $
    VALUE = 'plot region',ys=21)
button4 = WIDGET_BUTTON(base3, $
    UVALUE = 'region_statistic', $
    VALUE = 'region statistic',ys=21)
mean_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='mean value')
rms_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='rms value')

button5 = WIDGET_BUTTON(base3, $
    UVALUE = 'region_fwhm', $*
    VALUE = 'region centroid',ys=21)
POS_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='x    y')
FWHM_label = WIDGET_LABEL(base3, xs=150, $
    VALUE='FWHM')
FLUX_label=WIDGET_LABEL(base3,xs=150, $
    VALUE='flux')
button2 = WIDGET_BUTTON(base3, $
    UVALUE = 'DONE', $
    VALUE = 'DONE',ys=24)
micro = WIDGET_DRAW(base3, XSIZE=150 , YSIZE=150,/FRAME, $
    UVALUE = 'micro',   RETAIN = 1)

; Realize the widgets:

cgCENTERTLB,base_view
...
XMANAGER, "IFURED_viewspec", base_view, GROUP_LEADER=GROUP, /NO_BLOCK
END

pro IFURED_viewspec_load,filename
COMMON IFURED_viewspec_Wbase
COMMON IFURED_viewspec_IMAblock
COMMON IFURED_viewspec_RANGEblock
  ...
  nx=mm[1]+1-mm[0]
  zero_pix=mm[0]
  ima=congrid(frame(mm[0]:mm[0]+nx-1,*,plane),x_size,y_size)
