...
pro ifured_skylines_fit,filein,w_dir=w_dir, sysvel=sysvel,error=error, sn_min=sn_min
  ...
  lines_fit=[{line_par,'sky5577',5577.338, 0,0,0,0,5500.,5620.,0,0,0,0,'',''},$
             {line_par,'sky6300',6300.304,0,0,0,0,6250.,6350.,0,0,0,0,'',''}$
             ]
  ...
  cgps_open,maps_dir+'sky_fluxes.eps',xs=xs,ys=ys,/encaps
    for i=0,nmaps-1 do begin
  ...
  ;Velocity fields
  cgps_open,maps_dir+'sky_velocities.eps',xs=xs,ys=ys,/encaps
    for i=0,nmaps-1 do begin
      pos=[x0+(dx+xs_im)*(i mod grid[0]),y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1),$
              x0+(dx+xs_im)*(i mod grid[0])+xs_im,y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1)+ys_im]
      pos_cb=[pos[0],pos[3]+dycb,pos[2],pos[3]+dycb+yscb]
  ...
  ;Velocity Dispersion
  cgps_open,maps_dir+'sky_dispersions.eps',xs=xs,ys=ys,/encaps
    for i=0,nmaps-1 do begin
      pos=[x0+(dx+xs_im)*(i mod grid[0]),y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1),$
              x0+(dx+xs_im)*(i mod grid[0])+xs_im,y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1)+ys_im]
      pos_cb=[pos[0],pos[3]+dycb,pos[2],pos[3]+dycb+yscb]
  ...
  cgps_open,maps_dir+'sky_continuum.eps',xs=xs,ys=ys,/encaps
    for i=0,nmaps-1 do begin
      pos=[x0+(dx+xs_im)*(i mod grid[0]),y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1),$
              x0+(dx+xs_im)*(i mod grid[0])+xs_im,y0+(dy+ys_im+dycb+yscb)*(grid[1]-floor(i/grid[0])-1)+ys_im]
      pos_cb=[pos[0],pos[3]+dycb,pos[2],pos[3]+dycb+yscb]
