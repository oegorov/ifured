...
PRO IFURED_ADDEXP,names,out,filetest=filetest,w_dir=w_dir,T=T,lim=lim,flat=flat,no_cr=no_cr,lacosmic=lacosmic,$
  cut=cut,nonorm=nonorm,win=win,d_dir=d_dir,bias=bias,invert_overscan=invert_overscan,objtype=objtype,immask=immask,$
 ...
 if n_params() lt 2 then begin
  print,'IFURED_ADDEXP,names,out,[filetest=filetest,w_dir=w_dir,T=T,lim=lim,spectr=spectr,/flat,cut=cut,/nonorm,win=win,d_dir=d_dir,bias=bias,/invert_overscan,/shiftX,/ShiftY,XLIM=Xlim,Ylim=Ylim'
  return
 endif
...
cd,old_dir
 ...
 if keyword_set(d_dir) then cd,d_dir
 num=n_elements(names)
 Npair=num/2

 ; file size

  header=headfits(names[0])
  ...
  xs_slit=[separate[0]-cut[0]+1,xs-(separate[1]-cut[0])]
  xs_out=max(xs_slit)
  images=fltarr(xs_out,ys,2,num)
  images[*]=!Values.F_NAN
  sxaddhist,'ADDEXP: CH removed in '+names,header
  if keyword_set(bias) then sxaddhist,'Bias was removed',header


 Texp=fltarr(num)
 for i=0,num-1 do begin
    im=readfits(names[i],h)-imbias
    ;#### subtract scattering light in eta frames
    if objtype eq 'eta' then im=ifured_remove_bgr(im) 
    ...
    if keyword_set(bias) then begin
       ...
       print,names[i],'  Dif. of overscan :',over
       if abs(over) gt 20 then begin
        over=0
        print,'Too large! Set ovrescan=0!'
       endif
       im=float(im)-over
    endif

    ; cutting image
    images[0:separate[0]-cut[0],*,0,i]=im[cut[0]:separate[0],cut[1]:cut[3]]
    images[0:cut[2]-separate[1],*,1,i]=im[separate[1]:cut[2],cut[1]:cut[3]]
...
if keyword_set(shiftX) and num eq 1 and objtype eq 'obj' then begin
  ...
  yy1=1130 & yy2=1230
    ...
    for i=0,num-1 do begin
     ...
     sxaddhist,'IFURED_ADDEXP: '+names[i]+ ' slit '+string(slit,format="(I0)")+' was shifted on X:'+string(dx),header
...
;     wait,1
     ...
     sxaddhist,'ADDEXP: '+names[i]+ ' slit '+string(slit,format="(I0)")+' was shifted on Y:'+string(dy),header
...
if keyword_set(logfield) and (total(dx_shi) ne 0 or total(dy_shi) ne 0) then begin
  ...
  for i= start_i,num-1 do begin
  IFURED_LOG_UPD,[names[i]+":", "   Left slit dx="+string(dx_shi[0,i],format="(F0.5)")+"; dy="+string(dy_shi[0,i],format="(F0.5)")+";",$
...
if num eq 1 then begin ; One image!
  ...
  if not keyword_set(no_cr) then begin
    FOR i=0,Npair-1 do begin
     message,names[i*2]+'+'+names[i*2+1],/con
